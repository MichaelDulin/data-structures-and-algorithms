...
const addOne = (arr) => {
  let tempArr = [];
  arr.forEach(element => tempArr.push(element + 1));
  return tempArr;
};
...
const allUpperCase = (arr) => {
  let tempArr = [];
  arr.forEach(element => tempArr.push(element.toUpperCase()));
  return tempArr;
};
...
const createList = (availableItems) => {
  let newArr = [];
  availableItems.forEach(cur => {if (cur.available === true) newArr.push(cur.name);});
  return newArr;
};
