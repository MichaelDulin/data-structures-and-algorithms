...
const raisedToTheThird = (arr) => {
  let tempArr = [];
  arr.forEach(value => tempArr.push(Math.pow(value, 3)));
  return tempArr;
};
...
const addOne = (arr) => {
  const newArray = arr.map(value => value + 1);
  return newArray;
};
...
const forLoopTwoToThe = (arr) => {
  const newArray = arr.map(value => Math.pow(2, value));
  return newArray;
};
...
const forEachTwoToThe = (arr) => {
  let tempArr = [];
  arr.forEach(value => tempArr.push(Math.pow(2, value)));
  return tempArr;
};
