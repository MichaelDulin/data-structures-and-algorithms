...
const typeNum = (arr) => {
  let newArr = arr.filter(isNum => typeof isNum !== 'string');
  return newArr;
};
...
const oddValues = (arr) => {
  let newArr = arr.filter(isOdd => isOdd % 2 === 1);
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArr = arr.filter(includedVal => {
    return !forbiddenValues.includes(includedVal);
  });
  return newArr;
};
